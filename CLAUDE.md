...
2. **Anime-Lists mapping** (`anime-list.xml` from the Anime-Lists GitHub repo, `anime_mapping/`) — maps AniDB anime ids → TVDB series ids / TMDB movie ids; URL overridable via `kodi.metadata.animeMappingUrl`. Also cached in the CWD, but with a hardcoded 31-day TTL (mtime-based, `AnimeMappingLoader`).
