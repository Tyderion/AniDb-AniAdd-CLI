...
- `kodi/library/` (`feature/kodi-library-scan`) — after each batch, `KodiLibraryScanner` asks Kodi to scan the folders that received files (`kodi.libraryScan`, default off). `EpisodeProcessing` marks a `FileInfo` `libraryChanged` when it was actually moved or got an NFO that did not exist before; the scanner collects those via `addFileFinishedListener` and runs on `ProcessingEvent.Done`, and `AniAdd` waits for it before shutting down a `scan` run. `LibraryScanPlanner` (pure, unit-tested) maps local paths to Kodi paths per `localPath`; libraries given by `name` are resolved via `Files.GetSources`. Scans go through `aniAdd/kodi/KodiRpcClient` (id-matched replies, waits for `VideoLibrary.OnScanFinished` between scans) on a fresh websocket connection per batch, separate from the watched-state subscriber.
