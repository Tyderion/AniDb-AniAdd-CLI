...
- Test: `./gradlew test` (JUnit 5 + params + vintage engine, Mockito, Hamcrest). Config loading, path resolution, the renamer, the transcode queue and the tracked `.run/` configs are covered; the processing pipeline and the AniDB client are not, so don't assume tests catch regressions there.
...
- `transcode/` (`feature/transcode`) — a separate stage, never part of `EpisodeProcessing`: with `transcode.mode: queue` the pipeline only writes a `TranscodeJob` row (sqlite, keyed by the release's original ed2k+size) once a file is identified and moved; `anidb transcode` (no folder) runs `TranscodeRunner` as its own process, claiming jobs atomically under a renewed lease and walking committed phases `pending → encoding → encoded → swapped → done` so restarts resume without re-encoding. Order per job matters: the converted file is hashed and its `FileHashMapping` stored before the original is moved/deleted. Naming reuses `FileRenamer.targetPath`/`renameRelatedFiles` from cached `AniDBFileData`, no AniDB session. `anidb transcode <dir>` only queues. Tests `TranscodeRunnerTest`/`TranscoderTest` run real ffmpeg.
