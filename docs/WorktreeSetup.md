...
├── sandbox.yaml             settings: paths, gates, tag system
└── sandbox-*.yaml           one per task: scan, watch, watch-kodi, kodi
```

Everything in `sandbox/` is generated and untracked; the templates it is built from live in `gradle/`.
...
A config file splits in two. `sandbox.yaml` holds the settings and no run block; each `sandbox-<task>.yaml` holds only a run block and delegates with `config: sandbox.yaml`. That way there is exactly one entry point per task and one place that defines where things go.

`sandbox.yaml` is generated by merging `gradle/sandbox-overrides.yaml` onto `.run/scan-local.yaml`, so the tag system is defined once rather than copied. The run files are copied verbatim from `gradle/sandbox-runs/`. Adding a task means dropping another file in that folder. Its paths are relative (`input/`, `unknown/`, `../aniAdd.sqlite`), which works because relative paths in a config file resolve against that file. It is written with `file.mylist.add: false` and `anidb.exitOnBan: true` as hard gates, and `setupCheck` re-reads the file to confirm both are still set, since it is meant to be hand-edited. It is generated once and never overwritten without `-Pforce`.
